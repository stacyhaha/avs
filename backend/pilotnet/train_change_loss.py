...
def train(model: DrivingModel, optimizer: optim.Optimizer):
    ...
    for epoch in range(EPOCH):
        ...
        for i, data in enumerate(train_loader, 0):
            ...
            outputs = model(image)
            outputs = outputs * (torch.tensor([1.0, 10.0, 1000]).to(device))
            command_label = command_label * (torch.tensor([1.0, 10.0, 1000]).to(device))
            ...
            # print statistics
            # outputs = outputs * (torch.tensor([1.0, 1.0, 0.001]).to(device))
            # command_label = command_label * (torch.tensor([1.0, 1.0, 0.001]).to(device))
            # loss = criterion(outputs, command_label)
